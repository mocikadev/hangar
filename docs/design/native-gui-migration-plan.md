...
- 原生工程新增完整 macOS AppIcon Asset Catalog（16～1024 px）并沿用蓝色品牌资源；构建产物包含 `AppIcon.icns`，Info.plist 生成 `CFBundleIconName = AppIcon`。应用启动时把应用包图标显式赋给 `NSApp.applicationIconImage`，确保从 Accessory 恢复为 Regular 后 Dock 不回落为空白图标。菜单栏使用同一品牌标志的 18/36/54 px 单色模板 PNG：视觉与 Dock 图标一致，但遵循 macOS 状态栏不使用彩色方形底图的约定。
