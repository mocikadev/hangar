# GUI 版本设计（egui 第三前端，历史归档）

> 状态：0.6.0 已发布，源码已冻结并计划随原生 GUI 迁移删除；本文仅记录历史实现，不再作为后续 GUI 设计依据。新方向见 [`native-gui-migration-plan.md`](native-gui-migration-plan.md)。
...
- **OAuth 添加/复活**：后台跑登录流程；需手动粘贴时弹窗给出 URL（复制按钮）+
  粘贴输入框 + 确认/取消；添加成功仅入库不切换，重新登录成功只原位更新凭据且不改变当前账号，失败状态行显示原因。
