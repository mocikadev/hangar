...
use hangar_core::summary::AccountSummary;
...
fn safe_account(account: &Account, current: bool) -> Value {
    let now = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_secs();
    serde_json::to_value(AccountSummary::from_account(account, current, now))
        .expect("AccountSummary serialization cannot fail")
}
