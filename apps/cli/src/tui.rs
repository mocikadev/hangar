...
struct App {
    ...
    quota_attempts: HashMap<String, u64>,
...
impl App {
    ...
    fn load_quota_cache(&mut self) {
        let mut cache = core::quota_cache::load();
        for account in self.accounts.iter().filter(|account| !account.stale) {
            if let Some(entry) = cache.remove(&account.id) {
                self.quotas.insert(account.id.clone(), entry.quota);
                self.quota_times
                    .insert(account.id.clone(), entry.fetched_at);
            }
        }
    }

    fn fresh_quotas(&self) -> HashMap<String, Quota> {
        let now = core::quota_cache::now_secs();
        self.quotas
            .iter()
            .filter(|(id, _)| {
                self.quota_times.get(*id).is_some_and(|fetched_at| {
                    now >= *fetched_at && now - *fetched_at < core::quota_cache::TTL_SECS
                }) && !self.quota_errors.contains_key(*id)
            })
            .map(|(id, quota)| (id.clone(), quota.clone()))
            .collect()
    }
    ...
    fn apply_quota_result(
        ...
        self.quota_attempts
            .insert(id.clone(), core::quota_cache::now_secs());
        match result {
            Ok((account, quota)) => {
...
fn draw_ui(f: &mut ratatui::Frame, app: &mut App) {
    let area = f.area();
    let fresh_quotas = app.fresh_quotas();
    let recommendation = hangar_core::recommendation::recommend(
        &app.accounts,
    ...
    match app.selected_account() {
        ...
        Some(a) => match app.quotas.get(&a.id) {
            ...
            Some(q) => {
                ...
                if let Some(fetched_at) = app.quota_times.get(&a.id) {
                    let old = !app.fresh_quotas().contains_key(&a.id);
                    let suffix = if old { " · 旧数据" } else { "" };
                    f.render_widget(
                        Paragraph::new(format!(
                            "更新于 {}{}",
                            core::quota::fmt_ts_local(*fetched_at as i64),
                            suffix
                        )),
                        rows[2],
                    );
...
fn due_quota_targets(app: &App) -> Vec<(String, String)> {
    let cache = core::quota_cache::load();
    let now = core::quota_cache::now_secs();
    quota_targets(app)
        .into_iter()
        .filter(|(id, _)| {
            core::quota_cache::due(cache.get(id), now, false)
                && !app
                    .quota_attempts
                    .get(id)
                    .is_some_and(|last| now >= *last && now - *last < core::quota_cache::TTL_SECS)
        })
        .collect()
}
...
fn event_loop(term: &mut Terminal<CrosstermBackend<Stdout>>) -> Result<(), String> {
    ...
    let mut app = App {
        ...
        doctor_view: None,
        quotas: HashMap::new(),
        quota_times: HashMap::new(),
        quota_attempts: HashMap::new(),
    ...
    app.reload();
    app.load_quota_cache();
    // 账号规模以个位数为主：启动后单线程顺序查询缓存到期的正常账号，结果逐个回填；
    // 不引入并发刷新与额外锁竞争，网络任务也不阻塞浏览和退出。
    let startup = due_quota_targets(&app);
    if !startup.is_empty() {
        app.quota_busy = true;
        for (id, _) in &startup {
        ...
        while let Ok(ev) = app.rx.try_recv() {
            match ev {
                ...
                Ev::QuotaDone => {
                    ...
                    match hangar_core::recommendation::recommend(
                        &app.accounts,
                        app.current.as_deref(),
                        &app.fresh_quotas(),
                    ) {
                        Some(recommendation)
                            if recommendation.reason
                                == hangar_core::recommendation::Reason::KeepCurrent =>
                        {
                            app.push_log(format!(
                                "✅ 配额查询完成；建议继续使用 {}（周剩余 {}%）",
        ...
        Action::Reauth => {
            ...
            match target {
                ...
                Some((id, email)) => {
                    ...
                    match r {
                        Ok(_) => {
                            app.push_log(format!("✅ 已重新登录: {}（当前账号未改变）", email));
                        }
                        Err(e) => app.push_log(format!("✗ {}", e)),
                    }
                    core::account::harvest();
...
mod tests {
    ...
    fn overview_marks_weekly_recommendation_after_queries_finish() {
        ...
        app.quota_times
            .insert("id-1".into(), core::quota_cache::now_secs());
        app.quota_times
            .insert("id-2".into(), core::quota_cache::now_secs());
        let text = screen_text(&mut app);
    ...
    #[test]
    fn old_or_failed_quota_remains_visible_but_is_not_recommended() {
        let mut app = fake_app();
        app.accounts[1].stale = false;
        app.quotas.insert("id-1".into(), weekly_quota(40));
        app.quotas.insert("id-2".into(), weekly_quota(90));
        let now = core::quota_cache::now_secs();
        app.quota_times.insert("id-1".into(), now);
        app.quota_times
            .insert("id-2".into(), now - core::quota_cache::TTL_SECS);
        assert_eq!(app.fresh_quotas().len(), 1);
        let text = screen_text(&mut app);
        assert!(text.contains("90%"));
        assert!(text.contains('旧'));

        app.quota_times.insert("id-2".into(), now);
        app.quota_errors.insert("id-2".into(), "offline".into());
        assert_eq!(app.fresh_quotas().len(), 1);
        app.quota_errors.insert("id-1".into(), "offline".into());
        assert!(app.fresh_quotas().is_empty());
    }
