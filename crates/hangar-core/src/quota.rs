...
fn cache_success(account_id: &str, quota: &Quota) {
    if let Err(error) =
        crate::quota_cache::store_success(account_id, quota, crate::quota_cache::now_secs())
    {
        crate::emit::emit_err(format!("保存配额缓存失败: {error}"));
    }
}
