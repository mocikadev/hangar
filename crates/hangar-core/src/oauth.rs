...
pub fn parse_code_from_callback_url(url: &str, expected_state: &str) -> Result<String, String> {
    ...
    let port_ok = matches!(
        ...
        Some(CALLBACK_PORT | FALLBACK_CALLBACK_PORT)
    );
    if !host_ok || !port_ok || parsed.path() != "/auth/callback" {
        return Err(
...
fn callback_url_from_request(redirect_uri: &str, request_target: &str) -> Result<String, String> {
    Url::parse(redirect_uri)
        .and_then(|base| base.join(request_target))
        .map(String::from)
        .map_err(|error| format!("本地回调地址解析失败: {error}"))
}

fn authorization_url(redirect_uri: &str, state: &str, code_challenge: &str) -> String {
...
enum OAuthSessionOutcome {
    Waiting,
    Exchanging,
    Succeeded(Option<Account>),
    Failed(String),
    Cancelled,
    TimedOut,
}

/// UI-independent OAuth session. It owns the loopback listener and PKCE secret;
/// callers only receive the authorization URL and typed lifecycle state.
pub struct OAuthLoginSession {
    ...
    server: Arc<Server>,
    outcome: Mutex<OAuthSessionOutcome>,
}

impl OAuthLoginSession {
    pub fn start() -> Result<Arc<Self>, String> {
        Self::start_with_timeout(Duration::from_secs(300))
    }

    fn start_with_timeout(timeout: Duration) -> Result<Arc<Self>, String> {
        ...
        let session = Arc::new(Self {
            authorization_url: authorization_url(&redirect_uri, &state, &code_challenge),
            state,
            redirect_uri,
            code_verifier,
            server,
            outcome: Mutex::new(OAuthSessionOutcome::Waiting),
        });
        let weak = Arc::downgrade(&session);
        std::thread::spawn(move || Self::listen(weak, timeout));
        Ok(session)
    }
    ...
    pub fn phase(&self) -> OAuthSessionPhase {
        match &*self
            .outcome
            .lock()
            .unwrap_or_else(|error| error.into_inner())
        {
            OAuthSessionOutcome::Waiting => OAuthSessionPhase::Waiting,
            OAuthSessionOutcome::Exchanging => OAuthSessionPhase::Exchanging,
            OAuthSessionOutcome::Succeeded(_) => OAuthSessionPhase::Succeeded,
            OAuthSessionOutcome::Failed(_) => OAuthSessionPhase::Failed,
            OAuthSessionOutcome::Cancelled => OAuthSessionPhase::Cancelled,
            OAuthSessionOutcome::TimedOut => OAuthSessionPhase::TimedOut,
        }
    }
    ...
    pub fn error_message(&self) -> Option<String> {
        match &*self
            ...
            .unwrap_or_else(|error| error.into_inner())
        {
            OAuthSessionOutcome::Failed(message) => Some(message.clone()),
            _ => None,
        }
    }

    /// Accept a manually supplied callback URL. State and local callback path
    /// are validated before the one-shot exchange worker can be claimed.
    pub fn submit_callback(self: &Arc<Self>, callback_url: &str) -> Result<(), String> {
        let code = parse_code_from_callback_url(callback_url, &self.state)?;
        self.begin_exchange(code)
    }
    ...
    pub fn cancel(&self) -> bool {
        let mut outcome = self
            ...
            .lock()
            .unwrap_or_else(|error| error.into_inner());
        if !matches!(*outcome, OAuthSessionOutcome::Waiting) {
            return false;
        }
        *outcome = OAuthSessionOutcome::Cancelled;
        drop(outcome);
        self.server.unblock();
        true
    }

    /// Credentials never cross the FFI contract. The bridge consumes the
    /// account once and immediately commits it through Core account operations.
    pub fn take_account(&self) -> Option<Account> {
        let mut outcome = self
            .outcome
            .lock()
            .unwrap_or_else(|error| error.into_inner());
        match &mut *outcome {
            OAuthSessionOutcome::Succeeded(account) => account.take(),
            _ => None,
        }
    }

    fn begin_exchange(self: &Arc<Self>, code: String) -> Result<(), String> {
        let mut outcome = self
            .outcome
            .lock()
            .unwrap_or_else(|error| error.into_inner());
        if !matches!(*outcome, OAuthSessionOutcome::Waiting) {
            return Err("OAuth 会话已结束或正在处理".to_string());
        }
        *outcome = OAuthSessionOutcome::Exchanging;
        drop(outcome);
        self.server.unblock();
        let session = Arc::clone(self);
        std::thread::spawn(move || {
            let result =
                exchange_code_for_account(&code, &session.redirect_uri, &session.code_verifier);
            let mut outcome = session
                .outcome
                .lock()
                .unwrap_or_else(|error| error.into_inner());
            *outcome = match result {
                Ok(account) => OAuthSessionOutcome::Succeeded(Some(account)),
                Err(error) => OAuthSessionOutcome::Failed(error),
            };
        });
        Ok(())
    }

    fn listen(weak: Weak<Self>, timeout: Duration) {
        let deadline = Instant::now() + timeout;
        loop {
            let Some(session) = weak.upgrade() else {
                return;
            };
            if session.phase() != OAuthSessionPhase::Waiting {
                return;
            }
            ...
            if remaining.is_zero() {
                let mut outcome = session
                    ...
                    .unwrap_or_else(|error| error.into_inner());
                if matches!(*outcome, OAuthSessionOutcome::Waiting) {
                    *outcome = OAuthSessionOutcome::TimedOut;
                }
                return;
            }
            let wait = remaining.min(Duration::from_millis(250));
            let request = match session.server.recv_timeout(wait) {
                ...
                Err(error) => {
                    if session.phase() == OAuthSessionPhase::Waiting {
                        *session
                            ...
                            OAuthSessionOutcome::Failed(format!("本地回调服务失败: {error}"));
                    }
                    return;
                }
            };
            let callback_url = match callback_url_from_request(&session.redirect_uri, request.url())
            {
                Ok(url) => url,
                Err(error) => {
            ...
            match parse_code_from_callback_url(&callback_url, &session.state) {
                Ok(code) => {
                    let result = session.begin_exchange(code);
                    let status = if result.is_ok() { 200 } else { 409 };
                    let message = if result.is_ok() {
                        "授权成功！请关闭此窗口返回 Hangar。"
                    } else {
                        "OAuth 会话已结束"
                    };
                    let _ =
...
pub fn login_codex_with(hooks: &dyn LoginHooks) -> Result<Account, String> {
    ...
    let final_url = authorization_url(&redirect_uri, &state, &code_challenge);
    ...
    exchange_code_for_account(&code, &redirect_uri, &code_verifier)
}
...
fn exchange_code_for_account(
    ...
    let email =
        match crate::account::email_from_id_token(&token.id_token.clone().unwrap_or_default()) {
            Some(email) => email,
            None => get_user_email(&token.access_token).map_err(|error| {
                format!("无法确定账号邮箱（id_token 无 email 且 userinfo 失败: {error}）")
            })?,
        };
...
mod tests {
    ...
    fn callback_url_requires_localhost_auth_path() {
        ...
        .is_err());
        assert!(parse_code_from_callback_url(
            "http://localhost:9999/auth/callback?code=abc&state=s1",
            "s1"
        )
        .is_err());

        let reconstructed = callback_url_from_request(
            "http://localhost:1457/auth/callback",
            "/auth/callback?code=abc&state=s1",
        )
        .unwrap();
        assert_eq!(
            parse_code_from_callback_url(&reconstructed, "s1").as_deref(),
            Ok("abc")
        );
    }

    #[test]
    fn explicit_session_validates_state_cancel_and_timeout() {
        let session = OAuthLoginSession::start_with_timeout(Duration::from_secs(1)).unwrap();
        assert_eq!(session.phase(), OAuthSessionPhase::Waiting);
        let callback = format!(
            "{}/auth/callback?code=abc&state=wrong",
            session.redirect_uri.trim_end_matches("/auth/callback")
        );
        assert!(session.submit_callback(&callback).is_err());
        assert_eq!(session.phase(), OAuthSessionPhase::Waiting);
        assert!(session.cancel());
        assert!(!session.cancel());
        assert_eq!(session.phase(), OAuthSessionPhase::Cancelled);
        drop(session);
        std::thread::sleep(Duration::from_millis(20));

        let session = OAuthLoginSession::start_with_timeout(Duration::from_millis(5)).unwrap();
        for _ in 0..50 {
            if session.phase() == OAuthSessionPhase::TimedOut {
                return;
            }
            std::thread::sleep(Duration::from_millis(5));
        }
        panic!("OAuth session did not time out");
    }
