...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CliAuthCredentialsStore {
    File,
    Keyring,
    Auto,
}

/// 读取 Codex 顶层 `cli_auth_credentials_store` 配置。
///
/// 官方默认值是 `file`。这里只解析 Hangar 需要的一个顶层字符串键，避免为了
/// 单一兼容守卫引入完整 TOML 依赖；进入首个 table 后即停止，防止误认嵌套同名键。
fn parse_cli_auth_credentials_store(content: &str) -> Result<CliAuthCredentialsStore, String> {
    for line in content.lines() {
        ...
        let value = raw_value
            ...
            .ok_or_else(|| {
                "Codex 配置 cli_auth_credentials_store 必须是字符串 file、keyring 或 auto"
                    .to_string()
            })?;
        return match value {
            "file" => Ok(CliAuthCredentialsStore::File),
            "keyring" => Ok(CliAuthCredentialsStore::Keyring),
            "auto" => Ok(CliAuthCredentialsStore::Auto),
            other => Err(format!(
                "Codex 配置 cli_auth_credentials_store 的值不受支持：{other}"
            )),
        };
    }
    Ok(CliAuthCredentialsStore::File)
}

fn cli_auth_credentials_store(
    codex_home: &std::path::Path,
) -> Result<CliAuthCredentialsStore, String> {
    let path = codex_home.join("config.toml");
    match std::fs::read_to_string(&path) {
        Ok(content) => parse_cli_auth_credentials_store(&content),
...
/// Hangar 的账号库、收敛与诊断目前都以官方 auth.json 为唯一投影目标。
/// 对 keyring/auto 假装写成功会形成双权威，因此必须在任何刷新或写入前拒绝。
fn ensure_file_auth_store(codex_home: &std::path::Path) -> Result<(), String> {
    match cli_auth_credentials_store(codex_home)? {
        CliAuthCredentialsStore::File => Ok(()),
        mode @ (CliAuthCredentialsStore::Keyring | CliAuthCredentialsStore::Auto) => {
            let mode = match mode {
                CliAuthCredentialsStore::Keyring => "keyring",
                CliAuthCredentialsStore::Auto => "auto",
                CliAuthCredentialsStore::File => unreachable!(),
            };
            Err(format!(
                "当前 Codex 配置使用 cli_auth_credentials_store = \"{mode}\"，Hangar 暂只支持 file 模式；请改为 \"file\" 后重试。本次未切换账号，也未修改官方登录态"
            ))
        }
    }
}

fn ensure_projection_store_if_current(file: &AccountsFile, account_id: &str) -> Result<(), String> {
    if file.current_account_id.as_deref() == Some(account_id) {
        ensure_file_auth_store(&codex_home()?)?;
    }
    Ok(())
}
...
/// 重新登录不等于切换：非当前账号只更新账号库；当前账号按 S11 先更新账号库、
/// 再把新凭据同步到官方 auth.json，但 current_account_id 始终保持不变。
...
pub fn reauth_account_checked(target_id: &str, fresh: &Account) -> Result<(), AccountError> {
    ...
    let (was_current, updated) = update_reauthenticated_account(&mut file, target_id, fresh)?;
    persist_reauthenticated_account(
        &file,
        was_current,
        &updated,
...
fn update_reauthenticated_account(
    file: &mut AccountsFile,
    target_id: &str,
    fresh: &Account,
) -> Result<(bool, Account), AccountError> {
    let pos = file
        .accounts
        .iter()
    ...
    let updated = t.clone();
    let was_current = file.current_account_id.as_deref() == Some(target_id);
    Ok((was_current, updated))
}

fn persist_reauthenticated_account<Save, Project>(
    file: &AccountsFile,
    was_current: bool,
    updated: &Account,
    save: Save,
...
where
    Save: FnOnce(&AccountsFile) -> Result<(), String>,
    Project: FnOnce(&Account) -> Result<(), String>,
{
    save(file)?;
    if was_current {
        project(updated)?;
...
pub fn fresh_account(account_id: &str) -> Result<Account, String> {
    with_accounts_lock(|| {
        ...
        // 激活账号若刷新发生 token 轮换，随后必须投影官方存储；先确认该存储
        // 是 Hangar 支持的 file 模式，避免刷新并落库后才发现无法同步官方登录态。
        ensure_projection_store_if_current(&file, account_id)?;
...
fn switch_locked(account_id: &str) -> Result<(), AccountError> {
    ...
    // 存储模式必须在刷新（可能轮换 RT）和任何写入之前确认。显式 keyring/auto
    // 不能退化为只写 auth.json 后宣称成功，否则 Codex 与 Hangar 会读取不同权威。
    let codex_home =
        codex_home().map_err(|error| AccountError::new(AccountErrorKind::Internal, error))?;
    ensure_file_auth_store(&codex_home)
        .map_err(|error| AccountError::new(AccountErrorKind::State, error))?;
    ...
    project_official_auth_at(&codex_home, &account)
        .map_err(|error| AccountError::new(AccountErrorKind::Internal, error))?;
...
fn project_official_auth_at(codex_home: &std::path::Path, account: &Account) -> Result<(), String> {
    ensure_file_auth_store(codex_home)?;
    project_official_auth_to(codex_home, account)
}

fn project_official_auth_to(codex_home: &std::path::Path, account: &Account) -> Result<(), String> {
...
mod tests {
    ...
    #[test]
    fn cli_auth_store_defaults_to_file_and_only_reads_top_level_key() {
        assert_eq!(
            parse_cli_auth_credentials_store("# default config\nmodel = \"gpt-5\"\n").unwrap(),
            CliAuthCredentialsStore::File
        );
        assert_eq!(
            parse_cli_auth_credentials_store(
                "cli_auth_credentials_store = \"file\" # official default\n[profile.work]\n"
            )
            .unwrap(),
            CliAuthCredentialsStore::File
        );
    ...
    #[test]
    fn cli_auth_store_recognizes_unsupported_official_modes() {
        assert_eq!(
            parse_cli_auth_credentials_store("cli_auth_credentials_store = 'keyring'\n").unwrap(),
            CliAuthCredentialsStore::Keyring
        );
        assert_eq!(
            parse_cli_auth_credentials_store("cli_auth_credentials_store = \"auto\"\n").unwrap(),
            CliAuthCredentialsStore::Auto
        );
        assert!(
            parse_cli_auth_credentials_store("cli_auth_credentials_store = \"unknown\"\n").is_err()
        );
    }

    #[test]
    fn keyring_mode_refuses_projection_without_changing_auth_file() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!(
            "hangar-auth-store-test-{}-{unique}",
            std::process::id()
        ));
        ...
        let original = r#"{"sentinel":"unchanged"}"#;
        std::fs::write(&auth_path, original).unwrap();

        let error = project_official_auth_at(&dir, &test_account("blocked", "new"))
            .expect_err("keyring mode must be rejected");

        assert!(error.contains("暂只支持 file 模式"));
    ...
    #[test]
    fn reauth_inactive_account_updates_credentials_without_switching() {
        let mut target = test_account("inactive", "old");
        target.stale = true;
        let mut file = AccountsFile {
            accounts: vec![test_account("active", "active"), target],
            current_account_id: Some("active".into()),
        };
        let mut fresh = test_account("oauth-id", "fresh");
        fresh.email = "inactive@example.com".into();

        let (was_current, updated) =
            update_reauthenticated_account(&mut file, "inactive", &fresh).unwrap();

        assert!(!was_current);
        assert_eq!(file.current_account_id.as_deref(), Some("active"));
        assert_eq!(updated.id, "inactive");
        assert_eq!(updated.access_token, "at-fresh");
    ...
    #[test]
    fn reauth_current_account_preserves_current_identity() {
        let mut target = test_account("active", "old");
        target.stale = true;
        let mut file = AccountsFile {
            accounts: vec![target],
            current_account_id: Some("active".into()),
        ...
        let mut fresh = test_account("oauth-id", "fresh");
        fresh.email = "active@example.com".into();

        let (was_current, updated) =
            update_reauthenticated_account(&mut file, "active", &fresh).unwrap();

        assert!(was_current);
        assert_eq!(file.current_account_id.as_deref(), Some("active"));
        assert_eq!(updated.id, "active");
        assert_eq!(updated.refresh_token, "rt-fresh");
        assert!(!updated.stale);
